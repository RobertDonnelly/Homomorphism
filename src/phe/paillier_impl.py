...
def benchmark_operations(key_size: int = 2048, num_iterations: int = 10):
    """
    Benchmark Paillier operations for performance analysis.
    ...
    """
    print(f"\n{'='*60}")
    print(f"PAILLIER PERFORMANCE BENCHMARK ({key_size}-bit keys)")
    print(f"{'='*60}\n")
    ...
    for _ in range(num_iterations):
        enc = paillier_sys.encrypt(test_values[0])
    enc_time = (time.time() - start) / num_iterations
    print(f"Encryption:  {enc_time*1000:.2f} ms/operation")
    
    # Decryption time
    start = time.time()
    for _ in range(num_iterations):
        dec = paillier_sys.decrypt(enc)
    dec_time = (time.time() - start) / num_iterations
    print(f"Decryption:  {dec_time*1000:.2f} ms/operation")
    
    # Addition time
    enc1 = paillier_sys.encrypt(test_values[0])
    enc2 = paillier_sys.encrypt(test_values[1])
    start = time.time()
    for _ in range(num_iterations):
        result = paillier_sys.add_encrypted(enc1, enc2)
    add_time = (time.time() - start) / num_iterations
    print(f"Addition:    {add_time*1000:.2f} ms/operation")
    
    # Scalar multiplication time
    start = time.time()
    for _ in range(num_iterations):
        result = paillier_sys.multiply_encrypted_by_scalar(enc1, 2.5)
    mult_time = (time.time() - start) / num_iterations
    print(f"Scalar Mult: {mult_time*1000:.2f} ms/operation")
    
    print("\n--- Vector Operations (size=10) ---")
    # Vector encryption
    start = time.time()
    enc_vec = paillier_sys.encrypt_vector(test_vector)
    vec_enc_time = time.time() - start
    print(f"Vector Encryption:  {vec_enc_time*1000:.2f} ms")
    
    # Vector decryption
    start = time.time()
    dec_vec = paillier_sys.decrypt_vector(enc_vec)
    vec_dec_time = time.time() - start
    print(f"Vector Decryption:  {vec_dec_time*1000:.2f} ms")
